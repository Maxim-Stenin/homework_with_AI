"""Выгрузка CSV (SPEC п.9, п.12).

Против `tests/test_csv_export.py` не изменено ничего: выгрузку фикс не трогал,
все семь проверок были и остались зелёными. Файл скопирован целиком намеренно —
набор ретеста обязан покрывать ту же площадь, что и исходный, иначе «стало лучше»
может означать просто «стали меньше проверять».
"""
import pytest
