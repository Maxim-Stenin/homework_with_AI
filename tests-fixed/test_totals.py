...
Против `tests/test_totals.py` изменено: удаление идёт через `delete_row()`
(в фиксе оно спрашивает подтверждение). Ожидания не менялись.
"""
import pytest

from helpers import add, delete_row, legend, money_to_float, row_count, rows, totals
...
def test_invariant_totals_survive_add_delete_cycle(app):
    """Обратимость: добавили и удалили — итоги обязаны вернуться в исходные."""
    add(app, "100", category="Еда")
    before = totals(app)
    add(app, "555.55", category="Врем")
    # Индекс строки ищется по тексту, а не берётся нулевым: сортировка таблицы идёт
    # по дате, и у двух записей одного дня порядок ничем не гарантирован.
    index = next(i for i, r in enumerate(rows(app)) if "Врем" in r)
    delete_row(app, index=index)
    assert totals(app) == before, "цикл «добавили — удалили» не вернул итоги на место"
